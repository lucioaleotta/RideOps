...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const body = await request.json().catch(() => null) as { serviceId?: number } | null;
  if (!body?.serviceId) {
    return NextResponse.json({ message: 'Payload non valido' }, { status: 400 });
  }

  const backendUrl = process.env.BACKEND_URL ?? 'http://localhost:8080';
  const response = await fetch(`${backendUrl}/partners/${params.id}/communications/email`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`
    },
    body: JSON.stringify({ serviceId: Number(body.serviceId) }),
    cache: 'no-store'
  });
